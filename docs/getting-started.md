...
- `CLOSE_COOLIFY_REALTIME_PORTS`: controls host-level exposure of `6001/6002` through `DOCKER-USER` rules. `false` keeps direct public ingress possible; `true` blocks public ingress and enforces domain-based realtime path on `443`.
- `DOCKER_DISABLE_IPV6_FOR_PARSEADDR_FIX`: controls automatic mitigation for the known Coolify `Start Proxy` failure `ParseAddr(".../64")`. Default `true` applies Docker daemon workaround (`"ipv6": false`) when vulnerable Docker behavior is detected; `false` disables this automation.
...
- `COOLIFY_ROOT_USER_PASSWORD`: bootstrap/root login password for initial access. If placeholder/empty, `generate-secrets.*` creates one locally (minimum 16 chars including lowercase, uppercase, digit, and symbol).
...
- `CLOSE_COOLIFY_REALTIME_PORTS=true`: realtime uses effective domain on `443` (`COOLIFY_REALTIME_DOMAIN` or `COOLIFY_PUBLIC_DOMAIN` fallback); `DOCKER-USER` guards block direct public `6001/6002`.
...
1. Wait for first-boot VPS init/bootstrap to finish on the server (provider console):
   ...
   Note: `cloud-init` is the native Ubuntu first-boot service shipped by the VPS image.
   It is not created by this repository; this bootstrap runs inside that first-boot workflow.
...
5. Open Coolify and complete onboarding:
   ...
   First access uses `http://<SERVER_IP>:8000` (official installer flow).
   During onboarding, configure `COOLIFY_PUBLIC_DOMAIN`; final access becomes `https://<COOLIFY_PUBLIC_DOMAIN>`.
   ...
   Bootstrap replay also ensures root-user seeding (`RootUserSeeder`) and validates that the root account exists in Coolify DB.
...
- `Start Proxy` fails with `ParseAddr(".../64"): unexpected character`:
  bootstrap can auto-apply Docker workaround when `DOCKER_DISABLE_IPV6_FOR_PARSEADDR_FIX=true` (default).
  Replay bootstrap, then retry `Start Proxy`.
...
- Validate published web ports:
  ...
  - `8000` listens for initial Coolify access
  - `80/443` may not listen yet until onboarding/domain proxy setup is completed in UI
  - SSH on `SSH_PORT` is rate-limited for internet sources, while private Docker/internal source ranges are explicitly allowed (to keep Coolify localhost validation reliable)
