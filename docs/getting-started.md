...
| Variable | Runtime behavior | Default value | Autogenerated / source | Required to set? |
|---|---|---|---|---|
| `DEVOPS_USER` | Primary operational account; defaults to `devops` if not set | `devops` | No | NO |
| `SSH_KEY_ROTATE` | `0`: append SSH key; `1`: replace `authorized_keys` | `0` | No | NO |
...
| `COOLIFY_ROOT_USER_PASSWORD` | Installer root password | none (`CHANGE_ME` placeholder) | Yes: local pre-provision via `scripts/generate-secrets.sh` / `scripts/generate-secrets.ps1` when empty or `CHANGE_ME` | NO |

### C) Server user variables

| Variable | Runtime behavior | Default value | Autogenerated / source | Required to set? |
|---|---|---|---|---|
| `SSH_PUBLIC_KEY` or `SSH_PUBLIC_KEY_PATH` | Used for first SSH key injection and bootstrap key policy | `CHANGE_ME_or_leave_empty` / `CHANGE_ME_ssh_public_key` | Yes: local auto-detect from `~/.ssh/*.pub` by `scripts/generate-secrets.sh` / `scripts/generate-secrets.ps1` when empty/placeholder | YES (for direct SSH key-based access) |
...
| `COOLIFY_REALTIME_DOMAIN` | Dedicated realtime host; if set, bootstrap writes `PUSHER_HOST`, `PUSHER_PORT=443`, `PUSHER_SCHEME=https`; if empty, bootstrap removes those keys | empty | No | NO (YES when `CLOSE_COOLIFY_REALTIME_PORTS=true`) |
...
| Variable | Runtime behavior | Default value | Autogenerated / source | Required to set? |
|---|---|---|---|---|
| `USER_PASSWORDS_ENCRYPTION_PASSWORD` | Used to encrypt `/etc/vps-coolify-bootstrap/user-passwords.enc` | none (`CHANGE_ME` placeholder) | Yes: local pre-provision via `scripts/generate-secrets.sh` / `scripts/generate-secrets.ps1` when empty or `CHANGE_ME` | NO |
| account passwords for managed users | Password vault/runtime policy for `DEVOPS_USER`, `COOLIFY_SUDO_NOPASSWD_USER`, `ADDITIONAL_SUDO_USERS` | n/a | Yes: generated on VPS during bootstrap/replay by `scripts/ensure-user-passwords.sh` only when account is locked/unset or missing vault entry | YES (set your own local password for `DEVOPS_USER` on first login) |
