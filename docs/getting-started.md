...
| Variable | Runtime behavior | Default value | Autogenerated / source | Required |
|---|---|---|---|---|
| `DEVOPS_USER` | Primary operational account; defaults to `devops` if not set | `devops` | No | NO |
| `SSH_KEY_ROTATE` | `0`: append key for `DEVOPS_USER` + `ADDITIONAL_SUDO_USERS`; `1`: replace their `authorized_keys` with current `SSH_PUBLIC_KEY`; does not apply to `COOLIFY_SUDO_NOPASSWD_USER` | `0` | No | NO |
| `CLOSE_COOLIFY_REALTIME_PORTS` | `false`: keep public `6001/6002`; `true`: enforce `DOCKER-USER` guards and close public ingress | `false` | No | NO |

### B) Coolify admin variables

| Variable | Runtime behavior | Default value | Autogenerated / source | Required |
|---|---|---|---|---|
| `COOLIFY_PUBLIC_DOMAIN` | Public URL/domain used for onboarding and output | none (`CHANGE_ME` placeholder) | No | YES |
| `COOLIFY_ROOT_USERNAME` | Passed to Coolify installer | none (`CHANGE_ME` placeholder) | No | YES |
| `COOLIFY_ROOT_USER_EMAIL` | Passed to Coolify installer; login identifier is email | none (`CHANGE_ME` placeholder) | No | YES |
| `COOLIFY_ROOT_USER_PASSWORD` | Installer root password | none (`CHANGE_ME` placeholder) | Yes: local pre-provision via `scripts/generate-secrets.sh` / `scripts/generate-secrets.ps1` when empty or `CHANGE_ME` | YES |

### C) Server user variables

| Variable | Runtime behavior | Default value | Autogenerated / source | Required |
|---|---|---|---|---|
| `SSH_PUBLIC_KEY` or `SSH_PUBLIC_KEY_PATH` | Used for first SSH key injection and bootstrap key policy | `CHANGE_ME_or_leave_empty` / `CHANGE_ME_ssh_public_key` | Yes: local auto-detect from `~/.ssh/*.pub` by `scripts/generate-secrets.sh` / `scripts/generate-secrets.ps1` when empty/placeholder | NO (recommended YES for direct SSH key-based access) |
| `COOLIFY_SUDO_NOPASSWD_USER` | Dedicated user for Coolify localhost SSH operations; auto-managed, forced into user/group lists, granted passwordless sudo, and configured with a dedicated localhost/private-only SSH key (not for direct public SSH access) | `coolify` | No | NO |
| `ADDITIONAL_SUDO_USERS` | Optional additional admins separated by space, comma, or semicolon; effective managed users are `DEVOPS_USER` + `COOLIFY_SUDO_NOPASSWD_USER` + `ADDITIONAL_SUDO_USERS` | empty | No | NO |
| `COOLIFY_REALTIME_DOMAIN` | Dedicated realtime host; if set, bootstrap writes `PUSHER_HOST`, `PUSHER_PORT=443`, `PUSHER_SCHEME=https`; if empty, bootstrap removes those keys | empty | No | CONDITIONAL (YES when `CLOSE_COOLIFY_REALTIME_PORTS=true`, otherwise NO) |
| `SSH_PORT` | Applied in SSH config and service restart flow | `2222` | No | NO |
| `TIMEZONE` | Applied during first-boot VPS init phase | `UTC` | No | NO |

### D) Generated passwords and secrets

| Variable | Runtime behavior | Default value | Autogenerated / source | Required |
|---|---|---|---|---|
| `USER_PASSWORDS_ENCRYPTION_PASSWORD` | Used to encrypt `/etc/vps-coolify-bootstrap/user-passwords.enc` | none (`CHANGE_ME` placeholder) | Yes: local pre-provision via `scripts/generate-secrets.sh` / `scripts/generate-secrets.ps1` when empty or `CHANGE_ME` | YES |
| account passwords for managed users | Password vault/runtime policy for `DEVOPS_USER`, `COOLIFY_SUDO_NOPASSWD_USER`, `ADDITIONAL_SUDO_USERS` | n/a | Yes: generated on VPS during bootstrap/replay by `scripts/ensure-user-passwords.sh` only when account is locked/unset or missing vault entry | NO |
...
### Realtime routing matrix (detailed)

This matrix explains exactly what bootstrap enforces for realtime traffic.

| `CLOSE_COOLIFY_REALTIME_PORTS` | `COOLIFY_REALTIME_DOMAIN` | What bootstrap writes in Coolify `.env` | What bootstrap enforces at firewall/iptables level | Effective realtime path |
|---|---|---|---|---|
...
- Routing to realtime domain on `443` is application-level configuration (`PUSHER_*` values), not L4 NAT/port-forwarding.
- `CLOSE_COOLIFY_REALTIME_PORTS=false` does not disable domain routing if `COOLIFY_REALTIME_DOMAIN` is set; it only means direct public `6001/6002` remains allowed as well.
- UFW still allows `80/443` by baseline policy, so your reverse-proxy/domain path can work.

### `CLOSE_COOLIFY_REALTIME_PORTS=true` deep behavior
...
2. Reverse proxy/domain handling in Coolify is functional on `80/443`.
3. TLS certificate issuance for that domain succeeds.
