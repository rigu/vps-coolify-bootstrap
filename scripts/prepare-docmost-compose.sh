...
usage() {
  cat <<'USAGE'
Render Docmost compose template with values from docmost env.

Usage:
  scripts/prepare-docmost-compose.sh [--env-file <path>] [--template-file <path>] [--output-file <path>] [--overwrite]

Behavior:
  - If env file is missing, script creates parent directory and copies env/docmost-coolify.env.example.
  - Rewrites Docker Compose-style variables to `${VAR:-<value-from-docmost.env>}` defaults.
...
template_file="templates/docmost-coolify-compose.community.template.yml"
output_file="bootstrap-artifacts/docmost-coolify-compose.community.yml"
overwrite=0
...
bootstrap_info "prepare-docmost-compose parameters: env_file=${env_file}, template_file=${template_file}, output_file=${output_file}, overwrite=${overwrite}"
...
bootstrap_success "prepare-docmost-compose completed successfully."
